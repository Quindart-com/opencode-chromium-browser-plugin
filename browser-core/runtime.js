...
import { browserRequest, closeBrowserClients, listBrowserProfiles } from "../opencode-plugin/src/client.js";
...
const READ_LEGACY_TOOLS = new Set([
  ...
  "browser_download_events", "browser_snapshot", "browser_selected_tab", "browser_list_tabs",
]);
...
export class AgentBrowserRuntime {
  constructor({ artifactStore = new ArtifactStore(), approvalTtlMs = APPROVAL_TTL_MS } = {}) {
    this.artifacts = artifactStore;
    this.approvalTtlMs = approvalTtlMs;
    this.sessions = new Map();
    this.approvals = new Map();
    this.profileCache = { expiresAt: 0, profiles: [] };
    this.legacyToolsPromise = ChromiumBrowserPlugin().then((hooks) => hooks.tool);
  }
  ...
  async selectProfile(session, requested) {
    ...
    if (selector) {
      ...
      profile = profiles.find((item) => item.profileId === selector)
        ?? profiles.find((item) => item.profileLabel?.toLocaleLowerCase() === normalized);
      if (!profile) throw new Error(`Connected browser profile not found: ${selector}`);
    } else if (profiles.length === 1) {
      [profile] = profiles;
    } else {
      const error = new Error("Multiple browser profiles are connected; provide profile as an ID or exact label.");
  ...
  async executeStep(step, tabId, prior, session) {
    ...
    if (step.action === "find") {
      ...
      return this.invoke("browser_page_search", { tabId, query, maxResults: 20, detail: "lean", mode: "auto", timeoutMs }, session.sessionId);
    }
  ...
  async observeValue(args, tabId, session) {
    ...
    if (args.mode === "search") return this.invoke("browser_page_search", {
      ...
      timeoutMs: args.searchStrategy === "deep" ? 60000 : 10000,
    }, session.sessionId);
    ...
    if (args.mode === "extract") return target.selector
      ...
      : this.invoke("browser_page_search", { tabId, query: args.query ?? target.query ?? "main content", maxResults: limit, detail: args.detail ?? "compact", mode: args.searchStrategy ?? "auto", timeoutMs: 10000 }, session.sessionId);
  ...
  compact(payload, sessionId, maxChars = 4096, label = "result", inlinePayload = payload) {
    ...
    const artifact = this.artifacts.create({ sessionId, mimeType: "application/json", data: payload, label });
    ...
    return {
      ...
      sessionId,
      truncated: true,
      summary: payload.summary ?? `${label} was truncated to a useful inline preview; the complete result is optional at artifact.uri`,
      artifact,
    };
  }
  ...
  async session(args, context = {}) {
    ...
    try {
      ...
      return { ok: true, status: "ready", sessionId, profileId: profile.profileId, activeTabId: session.activeTabId, result: value };
    } catch (error) {
      return this.failure(sessionId, error);
    }
  }
  ...
  async finalize(args, context = {}) {
    ...
    try {
      ...
      return { ok: true, status: "finalized", sessionId, result };
    } catch (error) {
      return this.failure(sessionId, error);
    }
  }
