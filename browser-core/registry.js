...
const observationSchema = z.object({
  mode: z.enum(["search", "inspect", "visual", "extract", "events", "downloads", "screenshot", "raw-snapshot"]),
  target: targetSchema,
  query: z.string().optional(),
  detail: z.enum(["lean", "compact", "full", "debug"]).optional(),
  limit: z.number().int().positive().optional(),
  fullPage: z.boolean().optional(),
  searchStrategy: z.enum(["auto", "lexical", "deep"]).optional(),
}).optional();
...
const stepSchema = z.object({
  ...
  action: z.enum([
    ...
    "assert", "upload", "clipboardRead", "clipboardWrite", "screenshot", "close",
  ]),
  target: targetSchema,
